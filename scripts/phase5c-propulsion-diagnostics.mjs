  ...
  for (const probe of probes) {
    ...
    while (Date.now() < deadline) {
      ...
      if (
        ...
        sample.simulationTime >= lastSimulationTime + 0.25
      ) {
        samples.push(sample);
        lastSimulationTime = sample.simulationTime;
      }
      if (sample.calibrationReady) break;
